...
uniform float pn_invertFlip;

v2f vertNoFlip(appdata v)
{
    v2f o;
...
uniform sampler2D pn_ascii;

uniform float4 pn_bitsPerChannel;
uniform float3 pn_screenSize;
uniform sampler2D pn_orderedBayer;
uniform int pn_levels;

float4 Dither_Ordered(float4 rgba, float levels, float2 screenLoc)
{
    float bayer = tex2D(pn_orderedBayer, screenLoc).r;
    bayer = bayer * 2.0 - 1.0;
...
{
    return Dither_Ordered(float4(color, 1.0), levels, (uv * pn_screenSize.xy) / pn_screenSize.z);
}
ENDCG
...
Pass    // Do dithering, and do bit compression
{
    ...
    {
        float2 sampleUV = (floor(i.uv * pn_screenSize.xy) + 0.5f) / pn_screenSize;

		if (pn_bitsPerChannel.w < 0.0f)
		{
			float3 imageCol = tex2D(_MainTex, sampleUV).rgb;

			float3 offset = (1.0f / pn_bitsPerChannel) * 255.0f;
			offset = ceil(offset) / floor(offset);

			float4 c = Dither(imageCol / pn_bitsPerChannel, sampleUV, pn_levels);
		...
		float3 offset = (1.0f / pn_bitsPerChannel.w) * 255.0f;
		offset = ceil(offset) / floor(offset);
		pn_bitsPerChannel.xyz = pn_bitsPerChannel.www;
...
Pass    // Do bit compression only
{
    ...
    fixed4 frag (v2f i) : SV_Target
    {
		if (pn_bitsPerChannel.w < 0.0f)
		{
			...
			float4 c = float4(imageCol.rgb / pn_bitsPerChannel, imageCol.a);
			c.rgb = (floor(c * 255.0f) / 255.0f) * offset;
			c.rgb *= pn_bitsPerChannel;
...
Pass    // Do ascii
{
    ...
    fixed4 frag (v2f i) : SV_Target
    {
        ...
        float2 asciiUV = frac(i.uv * pn_screenSize.xy);
        ...
        float3 asciiCol = tex2D(pn_ascii, float2(luminance + asciiUV.x * widthX, asciiUV.y)).rgb;
...
Pass    // Do CRT
{
    ...
    #define res (float2(pn_screenSize.x/1.0, pn_screenSize.y/1.0))
    ...
    fixed4 frag (v2f i) : SV_Target
    {
        ...
        float2 screenUV = i.uv * pn_screenSize.xy * 3.0;
